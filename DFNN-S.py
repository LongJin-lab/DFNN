...
Node = collections.namedtuple('Node', ['id', 'input', 'extra', 'type'])
...
class Node_OP(nn.Module):
    def __init__(self, Node, inplanes, outplanes):
        ...
        if len(Node.extra) > 0:
            self.mean_weight = nn.Parameter(torch.ones(len(Node.extra)))
            self.sigmoid = nn.Sigmoid()

        if self.is_input_node:
            self.unit_op = Triplet_unit(inplanes, outplanes, stride=2)
        else:
            self.unit_op = Triplet_unit(outplanes, outplanes, stride=1)

    def forward(self, input, extra=[]):
        for i in range(0, len(extra)):
            # input = input + self.sigmoid(self.mean_weight[i])*extra[i]
            input = input + self.mean_weight[i]*extra[i]
        ...
        for id, node in enumerate(self.nodes):
            if node.id == self.source:
                ...
                res = self.nodeop[id](results[node.input],
                                      [results[ex] for ex in node.extra])
                results[node.id] = res
